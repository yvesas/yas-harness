...
describe('GitHubConnector — discussions', () => {
  ...
  it('creates a discussion, resolving the repo and category ids', async () => {
    ...
    expect(calls.map((c) => (/discussionCategories/.test(c.query) ? 'cat' : 'create'))).toEqual([
      'cat',
      'create',
    ]);
  });
    ...
    connect((op, vars) => {
      if (op === 'categories') {
        return {
          repository: {
            id: 'R',
            discussionCategories: { nodes: [{ id: 'FIRST', name: 'General' }] },
          },
        };
      }
      if (op === 'create') {
    ...
    connect((op) =>
      op === 'categories'
        ? {
            repository: {
              id: 'R',
              discussionCategories: { nodes: [{ id: 'G', name: 'General' }] },
            },
          }
        : undefined,
    );
  ...
  it('needs a repo to create a discussion', async () => {
    ...
    await expect(connector.create(ctx, { type: 'discussion', title: 'x' })).rejects.toThrowError(
      /needs metadata\.repo/,
    );
  });
  ...
  it('edits a discussion, reading its node id first then updating by id', async () => {
    connect((op, vars) => {
      if (op === 'read')
        return { repository: { discussion: discussionNode(3, 'Old title', 'old') } };
      if (op === 'update') {
    ...
    const updated = await connector.update(ctx, 'discussion:acme/widgets#3', {
      content: 'new body',
    });

    expect(updated.content).toBe('new body');
    // Read (for the node id) then update.
    expect(calls.map((c) => (/updateDiscussion/.test(c.query) ? 'update' : 'read'))).toEqual([
      'read',
      'update',
    ]);
  });
  ...
  it('routes issue ids to REST, discussion ids to GraphQL', async () => {
    ...
    connect((op) =>
      op === 'read' ? { repository: { discussion: discussionNode(1, 'D') } } : undefined,
    );
